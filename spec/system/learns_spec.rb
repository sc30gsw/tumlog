...
RSpec.describe "投稿削除", type: :system do
  ...
  context '投稿削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿した内容を削除できる' do
      ...
      expect{
        find_link("削除する", href: learn_path(@learn1)).click
      }.to change { Learn.count }.by(-1)
      # 投稿一覧ページに遷移する
      visit learns_path
      # 投稿一覧ページは投稿1の内容が存在しないことを確認する(画像)
      expect(page).to have_no_content(@learn1.image)
      # 投稿一覧ページは投稿1の内容が存在しないことを確認する(テキスト)
      expect(page).to have_no_content(@learn1.text)
    end
  end
  context '投稿削除できないとき' do
    it 'ログインしたユーザーは自分以外が投稿した内容を削除できない' do
      ...
      find('input[name="commit"]').click
      # 投稿2に削除ボタンがないことを確認する
      expect(page).to have_no_link '削除する', href: learn_path(@learn2)
    end
    it 'ログインしていないと投稿の削除ボタンがない' do
      ...
      visit learn_path(@learn1)
      # 投稿1に削除ボタンがないことを確認する
      expect(page).to have_no_link '削除する', href: learn_path(@learn1)
      # 該当カテゴリーページに遷移する
      visit learns_path
      # 投稿2の詳細ページに遷移する
      visit learn_path(@learn2)
...
RSpec.describe '投稿詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    expect(page).to have_selector("img")
    expect(page).to have_content(@learn.text)
    # コメント用のフォームが存在する
