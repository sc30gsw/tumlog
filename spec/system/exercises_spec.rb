...
RSpec.describe "新規投稿", type: :system do
  before do
    ...
    @exercise_text = Faker::Lorem.sentence
  end
  ...
  context '新規投稿ができるとき' do
    it 'ログインしたユーザーは新規投稿できる' do
      ...
      # textフォームに情報を入力する
      fill_in 'exercise_text', with: @exercise_text
      #添付する画像を定義する
      ...
      expect(page).to have_content(@exercise_text)
    end
  end
...
RSpec.describe "投稿編集", type: :system do
  ...
  context '投稿編集できるとき' do
    it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
      ...
      # 編集ページへ遷移する
      visit edit_exercise_path(@exercise1)
      # すでに投稿済みの内容がフォームに入っていることを確認する
      expect(page).to have_content(@exercise1.text)
      expect(page).to have_selector("img")
      # 投稿を編集する
      fill_in 'exercise_text', with: "#{@exercise1.text}+あいうえお"
...
RSpec.describe '投稿詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    expect(page).to have_selector("img")
    expect(page).to have_content(@exercise.text)
    # コメント用のフォームが存在する
